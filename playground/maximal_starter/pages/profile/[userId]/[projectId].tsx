...
const ProjectPage: NextPageWithAuthAndLayout = () => {
  ...
  const { state, dispatch } = trpcReducer.useTrpcReducer(
    projectReducer,
    ['project.active-project-detail', { id: Number(router.query.projectId) }],
    {
      arg_0: ['project.vote'],
      arg_1: ['project.undo-vote'],
      arg_2: ['comment.add'],
      arg_3: ['comment.delete'],
      arg_4: ['comment.edit'],
    }
  )
...
function AddCommentForm({ projectId }: { projectId: number }) {
  ...
  const addCommentMutation = trpc.useMutation('comment.add', {
    onSuccess: () => {
      return utils.invalidateQueries(['project.active-project-detail'])
    },
    onError: (error) => {
