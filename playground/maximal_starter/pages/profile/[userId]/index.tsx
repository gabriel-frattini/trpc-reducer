    ...
    <>
      ...
      <div className="flex justify-around">
        <p
          ...
          className={`${
            activeFeed === 'all'
              ? 'font-bold cursor-pointer bg-gray-100 p-2'
      ...
      {activeFeed == 'all' && <ProfileFeed />}
      {activeFeed == 'active' && <ActiveProjectsFeed />}
    </>
  )
}
...
function ProfileInfo() {
  ...
  if (profileQuery.data) {
    ...
    return (
      <>
        <Head>
          <title>{profileQuery.data.name} - sideclub</title>
        </Head>
...
function ActiveProjectsFeed() {
  const router = useRouter()
  const currentPageNumber = router.query.page ? Number(router.query.page) : 1

  const profileFeedQuery = trpc.useQuery(['project.active-feed'])

  if (profileFeedQuery.data) {
    const activeProjectCount = profileFeedQuery.data.activeProjects.length
        ...
        <div className="flow-root mt-28">
          {activeProjectCount === 0 ? (
            <div className="text-center text-secondary border rounded py-20 px-10">
              This user doesn&apos;t have any active projects yet.
            </div>
          ) : (
      ...
      <ul className="-my-12 divide-y divide-primary">
        {[...Array(3)].map((_, idx) => (
          <li key={idx} className="py-10">
            <ProjectSummarySkeleton hideOwner />
          </li>
        ))}
      </ul>
    </div>
...
function ProfileFeed() {
  ...
  const profileFeedQueryPathAndInput: InferQueryPathAndInput<'public.projects-feed'> =
    [
      'public.projects-feed',
      {
        ...getQueryPaginationInput(PROJECTS_PER_PAGE, currentPageNumber),
        ownerId: String(router.query.userId),
      },
    ]
  ...
  if (profileFeedQuery.data) {
    return (
      <>
        <div className="flow-root mt-28">
          {profileFeedQuery.data.allProjects.length === 0 ? (
            <div className="text-center text-secondary border rounded py-20 px-10">
              This user hasn&apos;t published any projects yet.
            </div>
          ) : (
            <ul className="-my-12 divide-y divide-primary">
              {profileFeedQuery.data.allProjects.map((project) => (
                <li key={project.id} className="py-10">
        ...
        <Pagination
          itemCount={profileFeedQuery.data.allProjects.length}
          itemsPerPage={PROJECTS_PER_PAGE}
          currentPageNumber={currentPageNumber}
        />
      </>
